...
ipcMain.on(
  ...
  (e: IpcMessageEvent, msg: string, ...args: any[]) => {
    sendToAllBackgroundPages(msg, ...args);
    // TODO: UI
...
ipcMain.on('emit-tabs-event', (e: any, name: string, ...data: any[]) => {
  ...
  sendToAllBackgroundPages(`api-emit-event-tabs-${name}`, ...data);
});

export const sendToAllBackgroundPages = (msg: string, ...args: any[]) => {
  for (const extension of Object.values(extensions)) {
    const { webContents } = extension.backgroundPage;
